...
(function (angular, undefined) {
	...
	.directive('rnStepper', function () {
		...
		template: '<div class="form-group">' +
		    '<button type="button" ng-disabled="isOverMax()" ng-click="increment()">' +
		    '<svg><use xlink:href="#chevron-up" /></svg>' +
		    '</button>' +
		    '<input type="text" ng-model="ngModel">' +
		    '<button type="button" ng-disabled="isOverMin()" ng-click="decrement()">' +
		...
		link: function (scope, iElement, iAttrs, ngModelController) {
			...
			return (angular.isDefined(
			    scope.min) && (ngModelController.$viewValue - offset) < parseInt(
			    scope.min, 10));
		    };
		    scope.isOverMax = function (strict) {
			var offset = strict ? 0 : 1;
			return (angular.isDefined(
			    scope.max) && (ngModelController.$viewValue + offset) > parseInt(
			    scope.max, 10));
		    };
